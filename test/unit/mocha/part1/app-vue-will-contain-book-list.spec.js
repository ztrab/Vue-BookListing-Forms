...
describe("App.vue", () => {
  it("should include booklist in App.vue @app-vue-will-use-book-list", () => {
    ...
    assert(
      importDeclaration[0].specifiers[0].local.name == "BookList",
      "App.vue isn't importing a class named `BookList`"
    );

    let results = esquery(
      ast,
      'ImportDeclaration[source.value="./components/BookList"]'
    );
    ...
    results = esquery(
      ast,
      "Property[key.name=components] > ObjectExpression > Property[key.name=BookList]"
    );
    assert(
      results.length > 0,
      "`BookList` is not defined as a value in an object in the `components` property"
    );
    ...
    assert(
      results.length == 1,
      "There doesn't appear to be a single `book-list` element with opening and closing tags inside of a div in App.vue."
    );
    assert(
      results,
      "The `<book-list>` element does not exist inside of a div in App.vue."
    );
  });
});
